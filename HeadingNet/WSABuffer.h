...
namespace Heading
{
	...
	public:
		WSABuffer	(						);
		WSABuffer	( uint64_t _sessionKey	);
		WSABuffer	( WSABuffer& _copy		);
		WSABuffer	( WSABuffer&& _move		);
		~WSABuffer	(						);

		void		commit		( uint64_t _length		);

		bool		get_buffer	( WSABUF** _buffer		);	// 버퍼가 읽을 수 있는 상태일 때 true와 함께 정보를 반환합니다.
		WSAHeader*	get_data	(						);	// 데이터를 한 개만 읽습니다.
		void		get_data	( WSApacketBuff* _datas	);	// 모든 데이터를 읽어서 꺼냅니다.
	...
	private:
		...
		WSABUF		m_buffer								= {};	// WSA에서 쓰는 버퍼 형태로 돌려주는 내용입니다.
